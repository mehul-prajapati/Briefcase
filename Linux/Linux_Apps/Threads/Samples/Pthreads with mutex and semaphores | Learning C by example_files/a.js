...
jQuery( function( $ ) {
	...
	/**
	 * Handles sliding the left sidebar.
	 */
	$('#page').delegate( '#tertiary-control', 'click', function( e ) {
		var sidebar = $('#tertiary');
		e.preventDefault();

		if ( sidebar.hasClass( 'open' ) ) {
			sidebar.removeClass( 'open' ).css( 'overflow', '' );
			$( this ).text( '+' ).attr( 'title', silesia.open_sidebar_title ).blur();
		} else {
			if ( sidebar.height() > window.innerHeight ) {
				sidebar.css( 'overflow', 'auto' );
			}
			sidebar.addClass( 'open' );
			$( this ).text( '-' ).attr( 'title', silesia.close_sidebar_title ).blur();
		}
	} );
} );;
var WPCOMSharing = {
