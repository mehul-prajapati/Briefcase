...
//if (window.location.href.search(/\/(mydeveloperworks|community)\/(files|wikis)/i) == -1) { 
	...
	xmlc.fetchData('/developerworks/lc/html/dwCustomHeader_'+langArg+'.html',
	...
	xmlc.fetchData('/developerworks/lc/html/dwCustomFooter_'+langArg+'.html',
	...
	var customFooterScriptsUrl = "/developerworks/lc/html/dwCustomFooterScripts_"+langArg+".html";
...
xmlc.loadLangCSS = function(){
    ...
    if (cookieVal != "en") {
        ...
        } else if (cookieVal == "pt_br" || cookieVal == "pt_BR") {
	...
	if(form){
		...
		for(var i=0; i<a_list.length; i++){
			...
			if(a_node.href.indexOf('?')==-1){
				...
				xmlc.UIModification_downloadDisclaimer_NodeReplace(a_node,url);
			}
		}
	}
...
xmlc.UIModification_downloadDisclaimer_NodeReplace=function(a_node, url, type){	
	...
	//a_node.href=escape('javascript:xmlc.UIModification_downloadConfirm(window,"open","'+url.replace(/"/g,/\\"/)+'",null,"'+type+'");';
	//a_node.href='javascript:xmlc.UIModification_downloadDisclaimer_NodeProcess(this)';
	a_node.href='javascript:void(0)';
	//a_node.onclick=function(){}	;
	/*a_node.onclick='xmlc.UIModification_downloadDisclaimer_NodeProcess(this)';
	a_node.target="_self";
	a_node.setAttribute('url',url);
	a_node.setAttribute('type',type);*/
