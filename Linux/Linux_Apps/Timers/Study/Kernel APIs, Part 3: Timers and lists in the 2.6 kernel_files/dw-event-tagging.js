		...
		bindEventLinks: function () {
			...
			if (window.location.href.indexOf('topics') == -1) {
				...
				if (dojo.query("div.ibm-col-1-1 #dwc-exp-c").length > 0){
					dwOpp.ev(window.location.hostname, "display tile", "dev@Pulse: A developer happening", window.location.href, "DWART", "Tile experiment");
				}		
				
				// C link
				dojo.query("div.ibm-col-1-1 #dwc-exp-c a").onclick(function(evt){
					dojo.stopEvent(evt);
					dwOpp.ev(window.location.hostname, this.href, "dev@Pulse: A developer happening", window.location.href, "DWART", "Tile experiment");
					setTimeout(dojo.hitch(this, function(){window.location = this.href;}), 800);
				});
