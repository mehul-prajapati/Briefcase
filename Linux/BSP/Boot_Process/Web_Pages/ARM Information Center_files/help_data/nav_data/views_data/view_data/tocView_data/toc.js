...
var expandedNodeClicks = 0;

function mouseClickHandler(e) {
  	
  	var clickedNode = getTarget(e);
	if (clickedNode == expandedNode)
	{
		expandedNodeClicks++;
	}
	else
	{
  	...
  	else
  	{
  		...
  		if (plus_minus != null)
  		{
	    	...
	    	{
	    		expandedNode = clickedNode;
	    		expandedNodeClicks = 1;
   				expand(plus_minus);
   			}
  		...
  		if (plus_minus != null)
  		{
    		if (isCollapsed(plus_minus)) 
  			...
  			else if (isExpanded(plus_minus))
  			{
  				if (clickedNode == expandedNode)
  				{
  					if ((isIE && (expandedNodeClicks > 1)) || (expandedNodeClicks > 2))
  					{
	  	  				collapse(plus_minus);
	  	  				expandedNodeClicks = 0;
	  	  			}
